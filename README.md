...
内置批量评测（`evaluate.py` / Showcase 一键），对测试集汇总 **4 个核心指标**：
...
python evaluate.py                       # ③ 批量评测 → evaluation_report.json
```
...
> 技术细节见 [`ARCHITECTURE.md`](ARCHITECTURE.md)（架构契约）与 [`CHANGELOG.md`](CHANGELOG.md)（版本与冻结策略）。
